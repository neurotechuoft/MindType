...
def eeg(signal=None, sampling_rate=1000., labels=None, show=True):
    """Process raw EEG biosignals and extract relevant signal features using
    default parameters.
...
def get_power_features(signal=None,
                       ...
                       size=0.25,
                       overlap=0.5):
    """Extract band power features from EEG biosignals.
...
def get_plf_features(signal=None, sampling_rate=1000., size=0.25, overlap=0.5):
    """Extract Phase-Locking Factor (PLF) features from EEG biosignals between all
    channel pairs.
