...
class HDF(object):
    ...
    def __init__(self, path=None, mode='a'):
        ...
        try:
            self._signals = self._file['biosignals']
        except KeyError:
            if mode == 'r':
                raise IOError(
                    "Unable to create 'biosignals' group with current file mode.")
            self._signals = self._file.create_group('biosignals')
    ...
    def del_signal_group(self, group=''):
        """Delete all biosignals in a file group.
    ...
    def list_signals(self, group='', recursive=False):
        ...
        group : str, optional
            Signal group.
        recursive : bool, optional
            It True, also lists biosignals in sub-groups.

        Returns
        -------
        biosignals : list
            List of (group, name) tuples of the found biosignals.
        ...
        for name, item in node.iteritems():
            if isinstance(item, h5py.Dataset):
                ...
                out.extend(self.list_signals(group=aux,
                                             recursive=True)['biosignals'])

        return utils.ReturnTuple((out,), ('biosignals',))
