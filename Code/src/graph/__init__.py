...
__all__ = (
'Graph', 'Plot', 'MeshLinePlot', 'MeshStemPlot', 'LinePlot', 'SmoothLinePlot',
'ContourPlot')
__version__ = '0.4-dev'
...
class Graph(Widget):
    ...
    def _update_labels(self):
        ...
        if xlabel:
            ...
            xlabel.pos = int(x + width / 2. - xlabel.width / 2.), int(
                padding + y)
...
class ContourPlot(Plot):
    ...
    def draw(self, *args):
        ...
        bl = (funcx(self.xrange[0]) - xmin) * ratiox + size[0], (
        funcy(self.yrange[0]) - ymin) * ratioy + size[1]
        tr = (funcx(self.xrange[1]) - xmin) * ratiox + size[0], (
        funcy(self.yrange[1]) - ymin) * ratioy + size[1]
        image.pos = bl
        w = tr[0] - bl[0]
        h = tr[1] - bl[1]
        image.size = (w, h)
        ...
        def make_contour_data(self, ts=0):
            ...
            for ii, t in enumerate(time):
                for jj, x in enumerate(position):
                    data[ii, jj] = sin(k * x + omega * t) + sin(
                        -k * x + omega * t) / ts
            return ((0, max(position)), (0, max(time)), data)

        def update_points(self, *args):
            self.plot.points = [(x / 10., cos(Clock.get_time() + x / 50.)) for x
                                in range(-500, 501)]
