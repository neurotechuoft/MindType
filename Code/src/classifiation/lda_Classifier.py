...
class CharacterClassification:
    def __init__(self, channels_data, expected_result):
        ...
        self.qda_classifier = QuadraticDiscriminantAnalysis()
    
    def train(self, training_data, dif_channels=False):
        self.lda_classifier.fit(training_data, self.training_results)
        self.qda_classifier.fit(training_data, self.training_results)

        

    def get_predictions(self, channels_data, row_col, expected_characters="", dif_channels=False):
        """
            ...
            for flash in range(180):
                ...
                for channel in range(self.num_channels):
                    ...
                    if dif_channels is True:
                        flash_confidence += self.qda_classifiers[channel].decision_function(to_predict)
                    else:
                        flash_confidence += self.qda_classifier.decision_function(to_predict)
    ...
    def predict_flash(self, to_predict, actual):
        return (self.lda_classifier.score(to_predict, actual),
            self.qda_classifier.score(to_predict, actual))
